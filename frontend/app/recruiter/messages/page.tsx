...
import { useAuthStore, SavedCandidate } from "@/lib/store";
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "new_message") {
        ...
        setMessages(prev => {
          // Only add if this message belongs to the currently viewed conversation
          // We'll use a ref to check the current conversation
          return [...prev, newMsg];
        });
      }
  ...
  const handleStartNewChat = async (candidate: SavedCandidate) => {
    setIsLoading(true);
    try {
      const conv = await chatApi.getOrCreateConversation(candidate.id);
      // Convert ConversationDetail to Conversation format
      const conversation: Conversation = {
        id: conv.id,
        other_user_id: conv.other_user_id,
        other_user_name: conv.other_user_name,
        other_user_github: conv.other_user_github,
        last_message: null,
    ...
    <div className="h-[calc(100vh-12rem)]">
      <div className="mb-6">
        ...
        <p className="text-zinc-500 dark:text-zinc-400">
          Chat with candidates you've connected with
        </p>
      </div>
          ...
          {selectedConversation ? (
            <>
              ...
              <div className="flex items-center gap-3 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
                ...
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-white font-semibold">
                ...
                <div>
                  ...
                  {selectedConversation.other_user_github && (
